 ...
 */
public class ProbabilityTable implements CategoricalDistribution, Factor, Serializable {
	 ...
	 * table.
	 * 
	 * @see aima.core.probability.util.ProbabilityTable#iterateOverTable(Iterator)
	 * @see aima.core.probability.util.ProbabilityTable#iterateOverTable(Iterator,
	...
	@Override
	public aima.core.probability.util.ProbabilityTable normalize() {
		double s = getSum();
		if (s != 0 && s != 1.0) {
			for (int i = 0; i < values.length; i++) {
				values[i] = values[i] / s;
	...
	public CategoricalDistribution divideBy(CategoricalDistribution divisor) {
		return divideBy((aima.core.probability.util.ProbabilityTable) divisor);
	}

	@Override
	public CategoricalDistribution multiplyBy(CategoricalDistribution multiplier) {
		return pointwiseProduct((aima.core.probability.util.ProbabilityTable) multiplier);
	}

	@Override
	public CategoricalDistribution multiplyByPOS(
			CategoricalDistribution multiplier, RandomVariable... prodVarOrder) {
		return pointwiseProductPOS((aima.core.probability.util.ProbabilityTable) multiplier, prodVarOrder);
	}
	...
	@Override
	public aima.core.probability.util.ProbabilityTable sumOut(RandomVariable... vars) {
		Set<RandomVariable> soutVars = new LinkedHashSet<RandomVariable>(
				this.randomVarInfo.keySet());
		for (RandomVariable rv : vars) {
			soutVars.remove(rv);
		}
		final aima.core.probability.util.ProbabilityTable summedOut = new aima.core.probability.util.ProbabilityTable(soutVars);
		if (1 == summedOut.getValues().length) {
			...
			final Object[] termValues = new Object[summedOut.randomVarInfo
					.size()];
			aima.core.probability.util.ProbabilityTable.Iterator di = new aima.core.probability.util.ProbabilityTable.Iterator() {
	...
	public aima.core.probability.util.ProbabilityTable divideBy(aima.core.probability.util.ProbabilityTable divisor) {
		if (!randomVarInfo.keySet().containsAll(divisor.randomVarInfo.keySet())) {
			throw new IllegalArgumentException(
					"Divisor must be a subset of the dividend.");
		}

		final aima.core.probability.util.ProbabilityTable quotient = new aima.core.probability.util.ProbabilityTable(randomVarInfo
				.keySet());
		...
		if (1 == divisor.getValues().length) {
			...
			final MixedRadixNumber qMRN = new MixedRadixNumber(0,
					quotient.radices);
			aima.core.probability.util.ProbabilityTable.Iterator divisorIterator = new aima.core.probability.util.ProbabilityTable.Iterator() {
				public void iterate(Map<RandomVariable, Object> possibleWorld,
						double probability) {
	...
	public aima.core.probability.util.ProbabilityTable pointwiseProduct(final aima.core.probability.util.ProbabilityTable multiplier) {
	...
	public aima.core.probability.util.ProbabilityTable pointwiseProductPOS(
            final aima.core.probability.util.ProbabilityTable multiplier, RandomVariable... prodVarOrder) {
		final aima.core.probability.util.ProbabilityTable product = new aima.core.probability.util.ProbabilityTable(prodVarOrder);
		if (!product.randomVarInfo.keySet().equals(
				SetOps.union(randomVarInfo.keySet(), multiplier.randomVarInfo
						.keySet()))) {
			throw new IllegalArgumentException(
					"Specified list detailing order of multiplier is inconsistent.");
		}
		...
		if (1 == product.getValues().length) {
			...
			aima.core.probability.util.ProbabilityTable.Iterator di = new aima.core.probability.util.ProbabilityTable.Iterator() {
				...
				public void iterate(Map<RandomVariable, Object> possibleWorld,
						double probability) {
					int term1Idx = termIdx(term1Values, aima.core.probability.util.ProbabilityTable.this,
							possibleWorld);
				...
				private int termIdx(Object[] termValues, aima.core.probability.util.ProbabilityTable d,
	...
	private class RVInfo implements Serializable {
