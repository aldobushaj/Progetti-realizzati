...
public class Plan<S, A> {
	 ...
	 */
	public aima.core.search.nondeterministic.Plan<S, A> getPlan(int step, S state) {
	 ...
	 */
	public aima.core.search.nondeterministic.Plan<S, A> prepend(A action) {
		actionSteps.add(0, action);
		return this;
	}

	/** Adds an if-state-then-plan statement at the end of the plan. */
	public void addIfStatement(S state, aima.core.search.nondeterministic.Plan<S, A> plan) {
	...
	private static class IfStatement<S, A> {
		...
		S state;
		aima.core.search.nondeterministic.Plan<S, A> plan;

		IfStatement(S state, aima.core.search.nondeterministic.Plan<S, A> plan) {
			this.state = state;
			this.plan = plan;
		}
		...
		aima.core.search.nondeterministic.Plan<S, A> getPlan() {
			return plan;
		}
