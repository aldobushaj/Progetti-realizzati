...
public final class Polyline2D implements IGeometric2D {
	...
	@Override
	public aima.core.util.math.geom.shapes.Polyline2D transform(TransformMatrix2D matrix) {
		Point2D[] vertexesNew = new Point2D[vertexes.length];
		for(int i=0;i<vertexes.length;i++) {
			vertexesNew[i] = matrix.multiply(vertexes[i]);
		}
		return new aima.core.util.math.geom.shapes.Polyline2D(vertexesNew,isClosed || (Util.compareDoubles(vertexesNew[0].getX(),vertexesNew[vertexes.length - 1].getX()) && Util.compareDoubles(vertexesNew[0].getY(), vertexesNew[vertexes.length - 1].getY())));
	}
}
